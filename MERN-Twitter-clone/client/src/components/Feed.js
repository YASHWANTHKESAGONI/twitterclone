...
function Feed() {
  ...
  async function populateTweets() {
    const req = await fetch("http://localhost:5005/feed", {
      headers: {
  ...
  async function addTweets(e) {
    e.preventDefault();
    const req = await fetch(`http://localhost:5005/feed?t=${tweetCount}`, {
      headers: {
        "x-access-token": localStorage.getItem("token"),
      },
    });
    ...
    <div>
      <div className="form-flex">
        <img
          ...
          src={`http://localhost:5005/images/${userAvatar}`}
        ></img>
        ...
        <form
          ...
          action="http://localhost:5005/feed"
